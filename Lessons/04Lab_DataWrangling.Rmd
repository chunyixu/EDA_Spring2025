...
NTL.phys.data.Tuesday.1999 <- filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake" )
...
NTL.phys.data.Separate <- separate(NTL.phys.data.Tuesday.1999, sampledate, c("Y", "m", "d"))

NTL.phys.data.E3. <- 
  NTL.phys.data.Separate %>% 
  filter (m == "07") 
...
NTL.phys.data.E4 <- 
  ...
  summarise(mean.sur.temp = mean(temperature_C)) 
...
NTL.phys.Irradiance.Data <- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")

#or
NTL.phys.2Irradiance <- 
NTL.phys.data %>% 
  pivot_longer(
    col=c(irradianceWater:irradianceDeck),
