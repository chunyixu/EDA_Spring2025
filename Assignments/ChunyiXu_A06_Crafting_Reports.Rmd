...
#Producing the table for Mean Particulates (2.5mm)
knitr::kable(summary_PM25, caption = "Mean Particulates (2.5mm)", col.names = c("County","$\\mu g/m^3$") , digits = 2)

#Producing the table for Mean Ozone
knitr::kable(summary_PM25, caption = "Mean Ozone", col.names = c("County","$\\mu g/m^3$"), digits = 2 )
```
...
```{r boxplot.2018, echo=FALSE, warning=FALSE, fig.align='left', fig.height=3, fig.cap= "2018 monthly Ozone levels"}
#Checking the class of the month, and the result is integer
# class(epa_data$Month)


# Changing the month to be a factor
epa_data$Month <- factor(epa_data$Month,levels=1:12,
                                   labels=month.abb)


#Making a subdataset for Year 2018
epa_data_2018 <- epa_data %>%
  filter(Year == 2018)

#Creating boxplots of the distribution of Ozone levels by month for 2018
Box.Ozone.2018 <-
  ggplot(epa_data_2018, aes(x = Month, y = Ozone))+
  geom_boxplot()+
  ylab(expression ("Concentration ("*mu*~g/m^3*")"))+
  labs (title = "Monthly Ozone concertrations (2018)")
print (Box.Ozone.2018)


```


```{r boxplot.2019, echo=FALSE, warning=FALSE, fig.align='left', fig.height=3, fig.cap= "2019 monthly Ozone levels"}
#Making a subdataset for Year 2019
epa_data_2019 <- epa_data %>%
  filter(Year == 2019)

#Creating boxplots of the distribution of Ozone levels by month for 2019
Box.Ozone.2019 <-
  ggplot(epa_data_2019, aes(x = Month, y = Ozone))+
  geom_boxplot()+
   ylab(expression ("Concentration ("*mu*~g/m^3*")"))+
  labs (title = "Monthly Ozone concertrations (2019)")
print (Box.Ozone.2019)
...
## Git Repository
https://github.com/chunyixu/EDA_Spring2025.git
