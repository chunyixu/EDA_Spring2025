...
```{r basics 1}
#1. Using the sequence code command to generate a sequence, while using "<-" to assign this sequence a name
Fiftyfive_sequence <- seq(1, 55, 5)

#2. Using the mean and the median summary statistic command to compute the mean and median of the sequence Fiftyfive_sequence. Assigning two outcomes "mean_Fiftyfive" and "median_Fiftyfive" respectively 
mean_Fiftyfive <- mean (Fiftyfive_sequence)
median_Fiftyfive <- median (Fiftyfive_sequence)

#3. Using a conditional statement to ask R to determine if the mean "mean_Fiftyfive" is greater than the median "median_Fiftyfive." The resule is "False"
...
```{r basics 2}
#5. Creating three vectors with four components for each and naming them according to the information they consist, which are "student_names", "test_scores", and "scholarship" respectively
...
#6.Using the class command to ask R to specify what kind of vectors are created and using a comment to label
...
#13a. Run the first function with the vector of test scores; this function does not work and shows that "Error in if (x > 50) { : the condition has length > 1" 
# Run_If_Else <- If_Else_Function (test_scores)
# Run_If_Else

#13b. Run the second function with the vector of test scores; this function works and shows that "Pass" "Pass" "Pass" "Pass"
